import React from "react";
import "./css/Menu.css";

export default class MenuList extends React.Component{
    render(){
        return (
                <div className="container">
                    <div className="MenuList">
                        <h1 className="product-categoria">
                            1 - BEBIDAS
                        </h1>
                        <ul>
                            <li className="line">
                                <img className="product-image" src="
                                data:image/png;base64,"/>
                                <button className="product-button">+</button>
    
                                <h1 className="product-title"> Nome do prato </h1>
                                <p className="product-descricao">Desc ri cao dodfds gsd fgs dfg dsf gsdf gsdf gdsfg dsf gdfgs sdfgfffff prato</p>
                                <h2 className="product-valor">R$ 100,00</h2>
                            </li>
    
                            <li className="line">
                                <img className="product-image" src="
                                data:image/png;base64,"/>
                                <button className="product-button">+</button>
    
                                <h1 className="product-title"> Nome do prato </h1>
                                <p className="product-descricao">Desc ri cao dodfds gsd fgs dfg dsf gsdf gsdf gdsfg dsf gdfgs sdfgfffff prato</p>
                                <h2 className="product-valor">R$ 100,00</h2>
                            </li>
    
                            <li className="line">
                                <img className="product-image" src="
                                data:image/png;base64,"/>
                                <button className="product-button">+</button>
    
                                <h1 className="product-title"> Nome do prato </h1>
                                <p className="product-descricao">Desc ri cao dodfds gsd fgs dfg dsf gsdf gsdf gdsfg dsf gdfgs sdfgfffff prato</p>
                                <h2 className="product-valor">R$ 100,00</h2>
                            </li>
                        
                            <li className="line">
                                <img className="product-image" src="
                                data:image/png;base64,"/>
                                <button className="product-button">+</button>
    
                                <h1 className="product-title"> Nome do prato </h1>
                                <p className="product-descricao">Desc ri cao dodfds gsd fgs dfg dsf gsdf gsdf gdsfg dsf gdfgs sdfgfffff prato</p>
                                <h2 className="product-valor">R$ 100,00</h2>
                            </li>
                        
                            <li className="line">
                                <img className="product-image" src="
                                data:image/png;base64,"/>
                                <button className="product-button">+</button>
    
                                <h1 className="product-title"> Nome do prato </h1>
                                <p className="product-descricao">Desc ri cao dodfds gsd fgs dfg dsf gsdf gsdf gdsfg dsf gdfgs sdfgfffff prato</p>
                                <h2 className="product-valor">R$ 100,00</h2>
                            </li>
                        
                            <li className="line">
                                <img className="product-image" src="
                                data:image/png;base64,"/>
                                <button className="product-button">+</button>
    
                                <h1 className="product-title"> Nome do prato </h1>
                                <p className="product-descricao">Desc ri cao dodfds gsd fgs dfg dsf gsdf gsdf gdsfg dsf gdfgs sdfgfffff prato</p>
                                <h2 className="product-valor">R$ 100,00</h2>
                            </li>
                        
                            <li className="line">
                                <img className="product-image" src="
                                data:image/png;base64,"/>
                                <button className="product-button">+</button>
    
                                <h1 className="product-title"> Nome do prato </h1>
                                <p className="product-descricao">Desc ri cao dodfds gsd fgs dfg dsf gsdf gsdf gdsfg dsf gdfgs sdfgfffff prato</p>
                                <h2 className="product-valor">R$ 100,00</h2>
                            </li>
                        
                            <li className="line">
                                <img className="product-image" src="
                                data:image/png;base64,"/>
                                <button className="product-button">+</button>
    
                                <h1 className="product-title"> Nome do prato </h1>
                                <p className="product-descricao">Desc ri cao dodfds gsd fgs dfg dsf gsdf gsdf gdsfg dsf gdfgs sdfgfffff prato</p>
                                <h2 className="product-valor">R$ 100,00</h2>
                            </li>
                        
                            <li className="line">
                                <img className="product-image" src="
                                data:image/png;base64,"/>
                                <button className="product-button">+</button>
    
                                <h1 className="product-title"> Nome do prato </h1>
                                <p className="product-descricao">Desc ri cao dodfds gsd fgs dfg dsf gsdf gsdf gdsfg dsf gdfgs sdfgfffff prato</p>
                                <h2 className="product-valor">R$ 100,00</h2>
                            </li>
                        
                            <li className="line">
                                <img className="product-image" src="
                                data:image/png;base64,"/>
                                <button className="product-button">+</button>
    
                                <h1 className="product-title"> Nome do prato </h1>
                                <p className="product-descricao">Desc ri cao dodfds gsd fgs dfg dsf gsdf gsdf gdsfg dsf gdfgs sdfgfffff prato</p>
                                <h2 className="product-valor">R$ 100,00</h2>
                            </li>
                        
                            <li className="line">
                                <img className="product-image" src="
                                data:image/png;base64,"/>
                                <button className="product-button">+</button>
    
                                <h1 className="product-title"> Nome do prato </h1>
                                <p className="product-descricao">Desc ri cao dodfds gsd fgs dfg dsf gsdf gsdf gdsfg dsf gdfgs sdfgfffff prato</p>
                                <h2 className="product-valor">R$ 100,00</h2>
                            </li>
                            
                            <li className="line">
                                <img className="product-image" src="
                                data:image/png;base64,"/>
                                <button className="product-button">+</button>
    
                                <h1 className="product-title"> Nome do prato </h1>
                                <p className="product-descricao">Desc ri cao dodfds gsd fgs dfg dsf gsdf gsdf gdsfg dsf gdfgs sdfgfffff prato</p>
                                <h2 className="product-valor">R$ 100,00</h2>
                            </li>
                            
                            <li className="line">
                                <img className="product-image" src="
                                data:image/png;base64,"/>
                                <button className="product-button">+</button>
    
                                <h1 className="product-title"> Nome do prato </h1>
                                <p className="product-descricao">Desc ri cao dodfds gsd fgs dfg dsf gsdf gsdf gdsfg dsf gdfgs sdfgfffff prato</p>
                                <h2 className="product-valor">R$ 100,00</h2>
                            </li>
                            
                            <li className="line">
                                <img className="product-image" src="
                                data:image/png;base64,"/>
                                <button className="product-button">+</button>
    
                                <h1 className="product-title"> Nome do prato </h1>
                                <p className="product-descricao">Desc ri cao dodfds gsd fgs dfg dsf gsdf gsdf gdsfg dsf gdfgs sdfgfffff prato</p>
                                <h2 className="product-valor">R$ 100,00</h2>
                            </li>
                            
                            <li className="line">
                                <img className="product-image" src="
                                data:image/png;base64,"/>
                                <button className="product-button">+</button>
    
                                <h1 className="product-title"> Nome do prato </h1>
                                <p className="product-descricao">Desc ri cao dodfds gsd fgs dfg dsf gsdf gsdf gdsfg dsf gdfgs sdfgfffff prato</p>
                                <h2 className="product-valor">R$ 100,00</h2>
                            </li>
                            
                            <li className="line">
                                <img className="product-image" src="
                                data:image/png;base64,"/>
                                <button className="product-button">+</button>
    
                                <h1 className="product-title"> Nome do prato </h1>
                                <p className="product-descricao">Desc ri cao dodfds gsd fgs dfg dsf gsdf gsdf gdsfg dsf gdfgs sdfgfffff prato</p>
                                <h2 className="product-valor">R$ 100,00</h2>
                            </li>
                            
                            <li className="line">
                                <img className="product-image" src="
                                data:image/png;base64,"/>
                                <button className="product-button">+</button>
    
                                <h1 className="product-title"> Nome do prato </h1>
                                <p className="product-descricao">Desc ri cao dodfds gsd fgs dfg dsf gsdf gsdf gdsfg dsf gdfgs sdfgfffff prato</p>
                                <h2 className="product-valor">R$ 100,00</h2>
                            </li>
                            
                            <li className="line">
                                <img className="product-image" src="
                                data:image/png;base64,"/>
                                <button className="product-button">+</button>
    
                                <h1 className="product-title"> Nome do prato </h1>
                                <p className="product-descricao">Desc ri cao dodfds gsd fgs dfg dsf gsdf gsdf gdsfg dsf gdfgs sdfgfffff prato</p>
                                <h2 className="product-valor">R$ 100,00</h2>
                            </li>
                        </ul>
                    </div>
    
                    <div className="MenuList">
                        <h1 className="product-categoria">
                            2 - SALGADOS
                        </h1>
                        <ul className="coll">
                            <li className="line">
                                <img className="product-image" src="
                                data:image/png;base64,"/>
                                <button className="product-button">+</button>
    
                                <h1 className="product-title"> Nome do prato </h1>
                                <p className="product-descricao">Desc ri cao dodfds gsd fgs dfg dsf gsdf gsdf gdsfg dsf gdfgs sdfgfffff prato</p>
                                <h2 className="product-valor">R$ 100,00</h2>
                            </li>
    
                            <li className="line">
                                <img className="product-image" src="
                                data:image/png;base64,"/>
                                <button className="product-button">+</button>
    
                                <h1 className="product-title"> Nome do prato </h1>
                                <p className="product-descricao">Desc ri cao dodfds gsd fgs dfg dsf gsdf gsdf gdsfg dsf gdfgs sdfgfffff prato</p>
                                <h2 className="product-valor">R$ 100,00</h2>
                            </li>
    
                            <li className="line">
                                <img className="product-image" src="
                                data:image/png;base64,"/>
                                <button className="product-button">+</button>
    
                                <h1 className="product-title"> Nome do prato </h1>
                                <p className="product-descricao">Desc ri cao dodfds gsd fgs dfg dsf gsdf gsdf gdsfg dsf gdfgs sdfgfffff prato</p>
                                <h2 className="product-valor">R$ 100,00</h2>
                            </li>
                        
                            <li className="line">
                                <img className="product-image" src="
                                data:image/png;base64,"/>
                                <button className="product-button">+</button>
    
                                <h1 className="product-title"> Nome do prato </h1>
                                <p className="product-descricao">Desc ri cao dodfds gsd fgs dfg dsf gsdf gsdf gdsfg dsf gdfgs sdfgfffff prato</p>
                                <h2 className="product-valor">R$ 100,00</h2>
                            </li>
                        
                            <li className="line">
                                <img className="product-image" src="
                                data:image/png;base64,"/>
                                <button className="product-button">+</button>
    
                                <h1 className="product-title"> Nome do prato </h1>
                                <p className="product-descricao">Desc ri cao dodfds gsd fgs dfg dsf gsdf gsdf gdsfg dsf gdfgs sdfgfffff prato</p>
                                <h2 className="product-valor">R$ 100,00</h2>
                            </li>
                        
                            <li className="line">
                                <img className="product-image" src="
                                data:image/png;base64,"/>
                                <button className="product-button">+</button>
    
                                <h1 className="product-title"> Nome do prato </h1>
                                <p className="product-descricao">Desc ri cao dodfds gsd fgs dfg dsf gsdf gsdf gdsfg dsf gdfgs sdfgfffff prato</p>
                                <h2 className="product-valor">R$ 100,00</h2>
                            </li>
                        
                            <li className="line">
                                <img className="product-image" src="
                                data:image/png;base64,"/>
                                <button className="product-button">+</button>
    
                                <h1 className="product-title"> Nome do prato </h1>
                                <p className="product-descricao">Desc ri cao dodfds gsd fgs dfg dsf gsdf gsdf gdsfg dsf gdfgs sdfgfffff prato</p>
                                <h2 className="product-valor">R$ 100,00</h2>
                            </li>
                        
                            <li className="line">
                                <img className="product-image" src="
                                data:image/png;base64,"/>
                                <button className="product-button">+</button>
    
                                <h1 className="product-title"> Nome do prato </h1>
                                <p className="product-descricao">Desc ri cao dodfds gsd fgs dfg dsf gsdf gsdf gdsfg dsf gdfgs sdfgfffff prato</p>
                                <h2 className="product-valor">R$ 100,00</h2>
                            </li>
                        
                            <li className="line">
                                <img className="product-image" src="
                                data:image/png;base64,"/>
                                <button className="product-button">+</button>
    
                                <h1 className="product-title"> Nome do prato </h1>
                                <p className="product-descricao">Desc ri cao dodfds gsd fgs dfg dsf gsdf gsdf gdsfg dsf gdfgs sdfgfffff prato</p>
                                <h2 className="product-valor">R$ 100,00</h2>
                            </li>
                        
                            <li className="line">
                                <img className="product-image" src="
                                data:image/png;base64,"/>
                                <button className="product-button">+</button>
    
                                <h1 className="product-title"> Nome do prato </h1>
                                <p className="product-descricao">Desc ri cao dodfds gsd fgs dfg dsf gsdf gsdf gdsfg dsf gdfgs sdfgfffff prato</p>
                                <h2 className="product-valor">R$ 100,00</h2>
                            </li>
                        
                            <li className="line">
                                <img className="product-image" src="
                                data:image/png;base64,"/>
                                <button className="product-button">+</button>
    
                                <h1 className="product-title"> Nome do prato </h1>
                                <p className="product-descricao">Desc ri cao dodfds gsd fgs dfg dsf gsdf gsdf gdsfg dsf gdfgs sdfgfffff prato</p>
                                <h2 className="product-valor">R$ 100,00</h2>
                            </li>
                            
                            <li className="line">
                                <img className="product-image" src="
                                data:image/png;base64,"/>
                                <button className="product-button">+</button>
    
                                <h1 className="product-title"> Nome do prato </h1>
                                <p className="product-descricao">Desc ri cao dodfds gsd fgs dfg dsf gsdf gsdf gdsfg dsf gdfgs sdfgfffff prato</p>
                                <h2 className="product-valor">R$ 100,00</h2>
                            </li>
                            
                            <li className="line">
                                <img className="product-image" src="
                                data:image/png;base64,"/>
                                <button className="product-button">+</button>
    
                                <h1 className="product-title"> Nome do prato </h1>
                                <p className="product-descricao">Desc ri cao dodfds gsd fgs dfg dsf gsdf gsdf gdsfg dsf gdfgs sdfgfffff prato</p>
                                <h2 className="product-valor">R$ 100,00</h2>
                            </li>
                            
                            <li className="line">
                                <img className="product-image" src="
                                data:image/png;base64,"/>
                                <button className="product-button">+</button>
    
                                <h1 className="product-title"> Nome do prato </h1>
                                <p className="product-descricao">Desc ri cao dodfds gsd fgs dfg dsf gsdf gsdf gdsfg dsf gdfgs sdfgfffff prato</p>
                                <h2 className="product-valor">R$ 100,00</h2>
                            </li>
                            
                            <li className="line">
                                <img className="product-image" src="
                                data:image/png;base64,"/>
                                <button className="product-button">+</button>
    
                                <h1 className="product-title"> Nome do prato </h1>
                                <p className="product-descricao">Desc ri cao dodfds gsd fgs dfg dsf gsdf gsdf gdsfg dsf gdfgs sdfgfffff prato</p>
                                <h2 className="product-valor">R$ 100,00</h2>
                            </li>
                            
                            <li className="line">
                                <img className="product-image" src="
                                data:image/png;base64,"/>
                                <button className="product-button">+</button>
    
                                <h1 className="product-title"> Nome do prato </h1>
                                <p className="product-descricao">Desc ri cao dodfds gsd fgs dfg dsf gsdf gsdf gdsfg dsf gdfgs sdfgfffff prato</p>
                                <h2 className="product-valor">R$ 100,00</h2>
                            </li>
                            
                            <li className="line">
                                <img className="product-image" src="
                                data:image/png;base64,"/>
                                <button className="product-button">+</button>
    
                                <h1 className="product-title"> Nome do prato </h1>
                                <p className="product-descricao">Desc ri cao dodfds gsd fgs dfg dsf gsdf gsdf gdsfg dsf gdfgs sdfgfffff prato</p>
                                <h2 className="product-valor">R$ 100,00</h2>
                            </li>
                            
                            <li className="line">
                                <img className="product-image" src="
                                data:image/png;base64,"/>
                                <button className="product-button">+</button>
    
                                <h1 className="product-title"> Nome do prato </h1>
                                <p className="product-descricao">Desc ri cao dodfds gsd fgs dfg dsf gsdf gsdf gdsfg dsf gdfgs sdfgfffff prato</p>
                                <h2 className="product-valor">R$ 100,00</h2>
                            </li>
                        </ul>
                    </div>
                </div>
            );
    }
    
}
